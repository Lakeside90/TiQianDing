...
import com.xkhouse.fang.house.adapter.OnsaleHouseAdapter;
...
public class HouseOnsaleView {
	...
	private OnsaleHouseAdapter adapter;
	private ArrayList<XKRoom> roomList = new ArrayList<XKRoom>();
	
	private OnsaleHouseTypeRequest request;
	...
	public HouseOnsaleView(Context context, String projectId, String houseType) {
	...
	private void initView() {
		rootView = LayoutInflater.from(context).inflate(R.layout.view_house_onsale, null);
	...
	public void fillData(ArrayList<XKRoom> roomList){
		...
		if(adapter == null){
			adapter = new OnsaleHouseAdapter(context, projectId, roomList);
	...
	private void startDataTask(int page, boolean showLoading){
		if(NetUtil.detectAvailable(context)){
			if(request == null){
				request = new OnsaleHouseTypeRequest(modelApp.getSite().getSiteId(), projectId, 
