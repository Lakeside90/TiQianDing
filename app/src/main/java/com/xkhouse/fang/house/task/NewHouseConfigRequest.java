 ...
 */
public class NewHouseConfigRequest {

	private String TAG = "MapNewHouseConfigRequest";
	private RequestListener requestListener;

	private String siteId;	//站点ID
	//配置项Key[价格  类型  排序 楼盘特色  户型 面积  开发商  销售状态 开盘时间]
	private String configName = "PRICE_RANGE$PROPERTY_TYPE$ORDER$FEATURE_LIST$BEDROOM$AREA_TYPE$KAIFASHANG$SALE_STATE$OPENING_TIME";
	...
	private ArrayList<CommonType> developerList = new ArrayList<CommonType>();      //开发商
    private ArrayList<CommonType> saleStateList = new ArrayList<CommonType>();		//销售状态
    private ArrayList<CommonType> openTimeList = new ArrayList<CommonType>();		//开盘时间

    public NewHouseConfigRequest(String siteId, RequestListener requestListener) {
		this.siteId = siteId;
		this.requestListener = requestListener;
	}

	//星空宝佣金排序
	public NewHouseConfigRequest(String siteId, String configName, RequestListener requestListener) {
		this.siteId = siteId; 
		...
		StringRequest request = new StringRequest(url, new Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                if(Constants.SUCCESS_CODE_OLD.equals(code)){
                    ...
                    dbService.insertDeveloperList(developerList, siteId);
                	dbService.insertSaleStateList(saleStateList, siteId);
                    dbService.insertOpenTimeList(openTimeList, siteId);
	...
	public void parseResult(String result) {
        ...
        try {
            ...
            if (jsonObject != null) {
	        	...
	        	JSONArray developerArray = dataObj.optJSONArray("KAIFASHANG");
	        	if (developerArray != null && developerArray.length() > 0) {
	                for (int i = 0; i < developerArray.length(); i++) {
	                	JSONObject typeJson = developerArray.getJSONObject(i);
	                	CommonType type = new CommonType();
	                	type.setId(typeJson.optString("enumId"));
	                	type.setName(typeJson.optString("enumName"));
                ...
                JSONArray openTimeArray = dataObj.optJSONArray("OPENING_TIME");
                if (openTimeArray != null && openTimeArray.length() > 0) {
                    for (int i = 0; i < openTimeArray.length(); i++) {
                        JSONObject typeJson = openTimeArray.getJSONObject(i);
                        CommonType type = new CommonType();
                        type.setId(typeJson.optString("enumId"));
                        type.setName(typeJson.optString("enumName"));
