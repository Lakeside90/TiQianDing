...
* @Description: 在售房源
* @author wujian  
* @date 2015-9-29 下午5:44:53
 */
public class OnsaleHouseTypeRequest {

	private String TAG = "OnsaleHouseTypeRequest";
	...
	public OnsaleHouseTypeRequest(String siteId, String pid, String houseType,
	...
	public void doRequest(){
		...
		String url = StringUtil.getRequestUrl(Constants.MAIN_HOUSE_LIST, params);
		Logger.d(TAG, url);
            ...
            if (jsonObject != null) {
	        	...
	        	if (roomArray != null && roomArray.length() > 0) {
	                for (int i = 0; i <= roomArray.length() - 1; i++) {
	                	...
	                	room.setChoiceId(roomJson.optString("choiceId"));
	                	room.setPid(roomJson.optString("pid"));
	                	room.setBuildNo(roomJson.optString("buildNo"));
	                	room.setFloor(roomJson.optString("floor"));
	                	room.setRoomNo(roomJson.optString("roomNo"));
	                	room.setBuildArea(roomJson.optString("buildArea"));
	                	room.setTotalPrice(roomJson.optString("totalPrice"));
	                	room.setPhotoPath(roomJson.optString("photoPath"));
	                	room.setProjectName(roomJson.optString("projectName"));
	                	room.setTypename(roomJson.optString("housename"));
	                	room.setHouseTitle(roomJson.optString("houseTitle"));
	                	roomList.add(room);
	                }
		        }
