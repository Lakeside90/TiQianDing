...
import com.xkhouse.fang.house.task.OnsaleHouseTypeRequest;
import com.xkhouse.fang.house.view.HouseOnsaleView;
...
* @Description: 在售房源（新房） 
* @author wujian  
* @date 2015-9-11 下午2:17:08
 */
public class OnsaleHouseActivity extends AppBaseActivity {
	...
	private List<HouseOnsaleView> onSaleViews = new ArrayList<HouseOnsaleView>();
	...
	protected void setContentView() {
		...
		setContentView(R.layout.activity_onsale_house);
	...
	private void fillData() {
		...
		for(int i = 0; i < houseTypeList.size(); i++){
			HouseOnsaleView view = new HouseOnsaleView(mContext, projectId, houseTypeList.get(i).getId());
			onSaleViews.add(view);
		}
		onSaleViews.get(0).fillData(roomList);
		...
		indicator.setOnPageChangeListener(new OnPageChangeListener() {
			...
			public void onPageSelected(int arg0) {
				if(arg0 < onSaleViews.size()){
					onSaleViews.get(arg0).refreshView();
				}
			}
	...
	class TabPageIndicatorAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup container, int position) {
			container.addView(onSaleViews.get(position).getView(), 0);
			return onSaleViews.get(position).getView();
		}
	...
	private void startDataTask() {
		if(NetUtil.detectAvailable(mContext)){
			OnsaleHouseTypeRequest request = new OnsaleHouseTypeRequest(modelApp.getSite().getSiteId(),
