 ...
 */
public class RentHouseListRequest {

	private String TAG = "RentHouseListRequest";
    ...
    public RentHouseListRequest(){
        super();
    }

	public RentHouseListRequest(String siteId, int page, int num, String searchContent, RequestListener requestListener) {
		this.siteId = siteId;
		this.page = page;
		this.num = num;
		this.searchContent = searchContent;
		this.requestListener = requestListener;
		url = Constants.RENT_HOUSE_LIST + "?siteId="+ siteId + "&page=" + page + "&num=" + num + searchContent;
	}
	...
	//地图找房，不分页
	public RentHouseListRequest(String siteId, String searchContent, RequestListener requestListener) {
		this.siteId = siteId;
		this.searchContent = searchContent;
		this.requestListener = requestListener;
		url = Constants.RENT_HOUSE_LIST + "?siteId="+ siteId + searchContent;
	}
		...
		StringRequest request = new StringRequest(url, new Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        ...
                        if(Constants.SUCCESS_CODE.equals(code) && houseList.size()>0){
                            ...
                            //缓存第一页没有筛选过的数据
                            if(isNeedCache()){
                                AppCache.writeRentHouseListJson(siteId, response);
                            }
                        }else{
                           message.obj = msg;
            ...
            if (jsonObject != null) {
	        	...
	        	if (jsonArray != null && jsonArray.length() > 0) {
	                for (int i = 0; i <= jsonArray.length() - 1; i++) {
	                	...
	                	house.setPorjectId(houseJson.optString("aid"));
	                	...
	                	house.setTitle(houseJson.optString("hiretitle"));
    ...
    public boolean isNeedCache(){
        if (page != 1){
            ...
            if (searchContent.contains("&area=") || searchContent.contains("&membertype=")
                    || searchContent.contains("&price=") || searchContent.contains("&sort=")
                    || searchContent.contains("&houseType=") || searchContent.contains("&areaZone=")
                    || searchContent.contains("&isShare=") || searchContent.contains("&fitment=")
