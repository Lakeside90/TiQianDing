package com.xkhouse.fang.app.push;
...
public class MyPushMessageReceiver extends PushMessageReceiver {
    ...
    public void onBind(Context context, int errorCode, String appid,
    	...
    	Preference.getInstance().writeBDChannelId(channelId);
    	Preference.getInstance().writeBDUserId(userId);
    ...
    public void onNotificationClicked(Context context, String title,
        ...
        if (!TextUtils.isEmpty(customContentString)) {
            ...
            try {
                customJson = new JSONObject(customContentString);
                if(customJson != null){
                	newsType = customJson.optString("newsType");
                	newsTitle = customJson.optString("newsTitle");
                	newsUrl = customJson.optString("newsUrl");
                	time = customJson.optString("time");
                }
		...
		if (StringUtil.isEmpty(newsType)) {
			...
			if("1".equals(newsType)){
				...
				Intent intent = new Intent(context.getApplicationContext(), DiscountDetailActivity.class);
				intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				data.putString("url", newsUrl);
				intent.putExtras(data);
				context.getApplicationContext().startActivity(intent);
...
//        updateContent(context, responseString);
    ...
    public void onUnbind(Context context, int errorCode, String requestId) {
        ...
        if (errorCode == 0) {
            // 解绑定成功
//        	Preference.getInstance().writeBDChannelId("");
//        	Preference.getInstance().writeBDUserId("");
        	Log.d("WJ", "onUnbind..... ");
        }
        // Demo更新界面展示代码，应用请在这里加入自己的处理逻辑
