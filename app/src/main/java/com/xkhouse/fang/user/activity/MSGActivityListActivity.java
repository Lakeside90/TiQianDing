...
import com.xkhouse.fang.user.adapter.MSGActivityAdapter;
...
public class MSGActivityListActivity extends AppBaseActivity {
	...
	private ArrayList<MSGActivity> activityList = new ArrayList<MSGActivity>();
	...
	private void fillData(){
		...
		if(adapter == null ){
			adapter = new MSGActivityAdapter(mContext, activityList);
				...
				listRequest = new MessageDetailListRequest("", DEVICE_ID,modelApp.getSite().getSiteId(),
					...
					public void sendMessage(Message message) {
						...
						case Constants.SUCCESS_DATA_FROM_NET:
							...
							ArrayList<MSGActivity> temp = (ArrayList<MSGActivity>) message.obj;
							//根据返回的数据量判断是否隐藏加载更多
							...
							if(isPullDown && activityList != null){
								activityList.clear();
								currentPageIndex = 1;
							}
							...
							fillData();
                            if (currentPageIndex > 1 && message.arg1 == activityList.size()){
	...
	class MyTimerTask extends TimerTask {
		...
		public void run() {
			for (int i = 0; i < activityList.size(); i++) {
				double nowTime = activityList.get(i).getNowTime() + 1000;
				activityList.get(i).setNowTime(nowTime);
			}
