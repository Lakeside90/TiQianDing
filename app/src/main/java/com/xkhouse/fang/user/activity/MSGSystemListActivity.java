...
* @author wujian  
* @date 2015-11-5 上午8:39:06
 */
public class MSGSystemListActivity extends AppBaseActivity {
	...
	protected void setContentView() {
		setContentView(R.layout.activity_msg_system_list);
	}
		...
		if (NetUtil.detectAvailable(mContext)) {
			if(listRequest == null){
				listRequest = new MessageDetailListRequest(modelApp.getUser().getUid(), DEVICE_ID,modelApp.getSite().getSiteId(),
						MessageCenterActivity.MSG_SYSTEM, page, pageSize, new RequestListener() {
				...
				listRequest.setData(modelApp.getUser().getUid(), DEVICE_ID,modelApp.getSite().getSiteId(),
						MessageCenterActivity.MSG_SYSTEM, page, pageSize);
			}
			if (showLoading){
                msg_listView.setVisibility(View.GONE);
                error_lay.setVisibility(View.GONE);
